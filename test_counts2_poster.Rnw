...
\begin{block}{GLM testing scheme}
...
The Wilson's confidence intervals~\citep{brown_2001} are calculated independently for every dPCR experiment. The Dunn -– \v{S}id\'{a}k correction ensures control of the family-wise error rate.
\end{block}
\vfill
...
All in-silico experiments were performed using newest method of simulating digital PCR data~\citep{jacobs_2014}.
\end{block}
\vfill
...
<<coverage, echo = FALSE, message=FALSE,fig.width=14,fig.height=11.3>>=
load("coverage.RData")
...
ggplot(m_coverage, aes(x = prop, y = coverage, fill = method)) +
  ...
  geom_hline(y = 0.95, colour = "blue", size = 2, linetype = 5) +
  poster_theme
  @

The dashed blue line marks 0.95 border.
